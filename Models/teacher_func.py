...
class Choice:
    def __init__(self, choice_id, text, is_correct):
        self.id = choice_id   
        self.text = text      # nội dung đáp án  
        self.is_correct = is_correct  # đúng hay sai
class Question:
    def __init__(self, question_id, text, points, qtype):
        ...
        self.type = qtype    
        self.choices = []      
    def add_choice(self, choice): #thêm đáp án
        self.choices.append(choice)
    def validate(self): #kiểm tra tính hợp lệ của câu hỏi
        if self.points <= 0:
            return False
        has_correct = False
...
class QuizManager:
    ...
    def add_quiz(self, quiz):
        #Thêm một quiz vào danh sách quản lý
        self.quizzes[quiz.quizid] = quiz
        
    def get_published_quizzes(self):
        #Lấy danh sách các quiz đã publish cho Student
...
class Quiz:
    def __init__(self, quiz_id, title, description, timelimit, maxattempts):
        ...
        self.questions = []           
    def add_question(self, question):# thêm câu hỏi
        self.questions.append(question)
    def grade(self, answers):
        total_score = 0
        correct_count = 0
        for q in self.questions:
            chosen = answers.get(q.id, None) # Lấy câu tl cho câu hỏi này
            if chosen is not None and q.is_answer_correct(chosen):
